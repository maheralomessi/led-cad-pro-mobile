...
import { generatePDFBytes } from './services/pdfGenerator';
import { analyzeDesignWithAI, getGeminiKey, setGeminiKey } from './services/geminiService';
import { exportTextFile, exportPdfFile } from './services/exportService';
  ...
  <div className="max-w-5xl mx-auto py-20 px-6 animate-fade-in text-center">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-20">
      <button 
        onClick={async () => {
          if (!result) return;
          await exportPdfFile(generatePDFBytes(result.contours, result.ledPoints, params, result.pixelToMm));
        }}
      ...
      <button 
        onClick={async () => {
          if (!result) return;
          await exportTextFile(generateSVG(result.contours, result.ledPoints, params, result.pixelToMm), 'svg');
        }}
      ...
      <button 
        onClick={async () => {
          if (!result) return;
          await exportTextFile(generateDXF(result.contours, result.ledPoints, params, result.pixelToMm), 'dxf');
        }}
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      const HISTORY_KEY = 'led_cad_history';
      try {
        const { value } = await Preferences.get({ key: HISTORY_KEY });
        const saved = value || localStorage.getItem(HISTORY_KEY);
  ...
  const saveToHistory = (res: AnalysisResult, p: DesignParams, img: string) => {
    ...
    setHistory(prev => {
      ...
      Preferences.set({ key: HISTORY_KEY, value: json }).catch(console.error);
      return updated;
    });
  };
        ...
        <nav className="px-10 py-6 border-b border-white/5 flex flex-row-reverse justify-between items-center glass-card sticky top-0 z-50">
          ...
          <div className="flex flex-row-reverse items-center gap-3">
            <button
              ...
              className="text-sm font-black text-white/80 hover:text-white bg-white/5 px-5 py-3 rounded-full border border-white/10 transition-all"
            >
              الإعدادات
            </button
            >
          ...
          <div className="flex flex-row-reverse items-center gap-2">
            <button onClick={() => setShowSettings(true)} className="text-sm font-black text-white bg-white/10 px-4 py-3 rounded-full">الإعدادات</button>
            <div className="flex flex-row-reverse items-center gap-2"><button onClick={() => setShowSettings(true)} className="text-sm font-black text-white bg-white/10 px-4 py-3 rounded-full border border-white/10">الإعدادات</button><div className="flex flex-row-reverse items-center gap-2"><button onClick={() => setShowSettings(true)} className="text-sm font-black text-white bg-white/10 px-4 py-3 rounded-full">الإعدادات</button><div className="flex flex-row-reverse items-center gap-2"><button onClick={() => setShowSettings(true)} className="text-sm font-black text-white bg-white/10 px-4 py-3 rounded-full">الإعدادات</button><button onClick={() => setStep(AppStep.WELCOME)} className="text-sm font-black text-red-500 bg-red-600/10 px-4 py-3 rounded-full">إلغاء</button></div></div></div>
          </div>
          </div>
          </div>
          </div>
